...
func (sr *SessionRunner) checkStopConditions() bool {
	// Check stop-at-ratio
	if sr.session.StopAtRatio && sr.session.Downloaded > 0 {
		currentRatio := float64(sr.session.Uploaded) / float64(sr.session.Downloaded)
...
func (sr *SessionRunner) announce(event protocol.AnnounceEvent) {
	...
	sr.log("info", "Announcing to %s (event=%s, up=%d, down=%d, left=%d)",
		trackerURL, event, sr.session.Uploaded, sr.session.Downloaded, sr.session.Left)
	...
	if err != nil {
		sr.session.LastError = err.Error()
		sr.log("error", "Announce failed: %v", err)
		sr.onUpdate(sr.session)
		return
	}

	if resp.FailureReason != "" {
		sr.session.LastError = resp.FailureReason
		sr.log("error", "Tracker returned failure: %s", resp.FailureReason)
		sr.onUpdate(sr.session)
		return
	}
